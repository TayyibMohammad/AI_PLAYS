...
// ******************************COLLISION_DETECTION**************************************************************
let collided = false;
function collisionDetection(pointer, lowerColumnsCoord, upperColumnsCoord) {
    let pointerCoord = {
...
let Xvelocity = 30;
let Yvelocity = 0;
const input = document.querySelector("#Xvelocity");
input.addEventListener("input", () => {
    Xvelocity = parseInt(input.value);
});
const input2 = document.querySelector("#Yvelocity");
...
genColoumns();
// ************************************************GEN_COLLOUMNS_ENDED******************************************
const lowerColoumns = document.querySelectorAll(".lower-coloumn");
let PATH_WIDTH = lowerColoumns[cN - 1].getBoundingClientRect().x - lowerColoumns[0].getBoundingClientRect().x;
// ***************COORDS OF COLOUMNS**************************************************************
let lowerColumnsCoord = [];
let upperColumnsCoord = [];
const getCoordsLower = () => {
    const lowerColoumns = document.querySelectorAll(".lower-coloumn");
    // console.log(lowerColoumns);
    lowerColoumns.forEach(lcoloumn => {
        const coord = { X: lcoloumn.getBoundingClientRect().x, Y: lcoloumn.getBoundingClientRect().y };
        lowerColumnsCoord.push(coord);
    });
};
getCoordsLower();
const getCoordsUpper = () => {
    const lowerColoumns = document.querySelectorAll(".upper-coloumn");
    // console.log(lowerColoumns);
...
const movePointer = async () => {
    ...
    while (pointerX <= DESTx) {
        const result = collisionDetection(pointer, lowerColumnsCoord, upperColumnsCoord);
        if (result !== -1) {
            const scoreCard = document.querySelector("#user-score");
            const score = pointerX;
...
function output(x, layer1_wts, layer1_bia, layer2_wts, layer2_bia, layer3_wts, layer3_bia) {
...
const aiMovePointer = async () => {
    ...
    while (pointerX <= DESTx) {
        ...
        let result = collisionDetection(pointer, lowerColumnsCoord, upperColumnsCoord);
        if (result !== -1 || (pointerY >= 300 || pointerY <= 230)) {
            const scoreCard = document.querySelector("#ai-score");
            const iterCard = document.querySelector("#iter");
            iteration++;
            const current_score = pointerX;
            ...
            let testScorePromise = tester(test_wts1, test_bia1, test_wts2, test_bia2, test_wts3, test_bia3);
            iteration++;
            let testScore = await testScorePromise;
        ...
        const slider = document.getElementById("Yvelocity");
...
const tester = async (test_wts1, test_bia1, test_wts2, test_bia2, test_wts3, test_bia3) => {
    const pointer = document.querySelector("#pointer");
